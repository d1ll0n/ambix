...
describe("copyTasksDir", () => {
  ...
  it("copies the tasks dir from source to new session (not a symlink)", async () => {
    writeTask(dir, "00000000-0000-0000-0000-000000000001", {
      id: "1",
      subject: "do thing",
      status: "pending",
    });
    ...
    const result = await copyTasksDir({
      origSessionId: "00000000-0000-0000-0000-000000000001",
      newSessionId: "00000000-0000-0000-0000-000000000002",
      tasksBaseDir: dir,
    });

    expect(result.copiedTo).toBe(path.join(dir, "00000000-0000-0000-0000-000000000002"));
    expect(result.source).toBe(
      path.resolve(path.join(dir, "00000000-0000-0000-0000-000000000001"))
    );
    // It's a real directory, not a symlink
    expect(lstatSync(path.join(dir, "00000000-0000-0000-0000-000000000002")).isDirectory()).toBe(
      true
    );
    expect(lstatSync(path.join(dir, "00000000-0000-0000-0000-000000000002")).isSymbolicLink()).toBe(
      false
    );

    // Both sides contain the task
    expect(existsSync(path.join(dir, "00000000-0000-0000-0000-000000000001", "1.json"))).toBe(true);
    expect(existsSync(path.join(dir, "00000000-0000-0000-0000-000000000002", "1.json"))).toBe(true);
  });
  ...
  it("new session is independent — mutating the source after copy does not leak", async () => {
    ...
    writeTask(dir, "00000000-0000-0000-0000-000000000001", {
      id: "3",
      subject: "added after copy",
      status: "pending",
    });

    // New session still sees only the two tasks from copy time
    const newTasks = await listTasks("00000000-0000-0000-0000-000000000002", dir);
    expect(newTasks).toHaveLength(2);
    expect(newTasks.map((t) => t.id)).toEqual(["1", "2"]);

    // Source has all three
    const origTasks = await listTasks("00000000-0000-0000-0000-000000000001", dir);
  ...
  it("no-op when the source has no tasks dir", async () => {
    const result = await copyTasksDir({
      origSessionId: "00000000-0000-0000-0000-000000000003",
      newSessionId: "00000000-0000-0000-0000-000000000002",
      tasksBaseDir: dir,
    });
    ...
    expect(existsSync(path.join(dir, "00000000-0000-0000-0000-000000000002"))).toBe(false);
  });

  it("copies .highwatermark and task payloads together so IDs stay consistent", async () => {
    const origDir = path.join(dir, "00000000-0000-0000-0000-000000000001");
  ...
  it("rejects non-UUID session IDs to guard against path traversal", async () => {
    // A crafted session file could set sessionId to e.g. "../../etc", which
    // fs.cp would happily copy from outside the tasks dir. CC only ever
    // emits UUIDs, so rejecting non-UUIDs is strictly safe.
    await expect(
      copyTasksDir({
        origSessionId: "../../etc",
        newSessionId: "00000000-0000-0000-0000-000000000002",
    ...
    ).rejects.toThrow(/non-UUID origSessionId/);
    ...
    ).rejects.toThrow(/non-UUID newSessionId/);
  });
  ...
  it("throws rather than overwriting an existing destination", async () => {
    ...
    const destDir = path.join(dir, "00000000-0000-0000-0000-000000000002");
